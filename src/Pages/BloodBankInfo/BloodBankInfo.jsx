...
const BloodBankPage = () => {
  ...
  const divisions = [
    "All",
    ...new Set(bloodBanks.map((bank) => bank.division)),
  ];
  ...
  return (
    <MyContainer>
      <h2 className="text-xl lg:text-2xl font-bold my-5 px-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-2 md:gap-5 my-5 px-2">
        {filteredBanks?.length === 0 ? (
          ...
          filteredBanks?.map((bank, idx) => (
            <div
              key={idx}
              className="p-2 md:p-3 p-border rounded-lg shadow hover:shadow-lg transition"
            >
              <h3 className="text-lg font-semibold mb-2">{bank.name}</h3>
              <p>
                <span className="text-sm font-semibold">Division:</span> {bank.division}
              </p>
              <p>
                <span className="text-sm font-semibold">District:</span> {bank.district}
              </p>
              <p>
                <span className="text-sm font-semibold">Address:</span> {bank.address}
              </p>
              <p>
                <span className="text-sm font-semibold">Phone:</span> {bank.phone}
              </p>
              <p>
                <span className="text-sm font-semibold">Email:</span> {bank.email}
              </p>
            </div>
          ))
        )}
